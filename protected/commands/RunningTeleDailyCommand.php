<?php
class RunningTeleDailyCommand extends CConsoleCommand
{
	protected $pdf;
		...
		try
		{
			...
			foreach($dataReader1 as $row1)
			{
		        ...
		        $this->pdf->row(array('1','PIUTANG DAGANG 1 - 30',
					...
					Yii::app()->numberFormatter->formatCurrency(($row9['0sd30']/($row9['0sd30']+$row9['31sd60']+$row9['61sd90']+$row9['91sd120']+$row9['up120']))* 100,''),
		        ));
		        $this->pdf->row(array('2','PIUTANG DAGANG 31 - 60',
					Yii::app()->numberFormatter->formatCurrency($row9['31sd60']/$per,'Rp. '),
					Yii::app()->numberFormatter->formatCurrency(($row9['31sd60']/($row9['0sd30']+$row9['31sd60']+$row9['61sd90']+$row9['91sd120']+$row9['up120']))*100,''),
		        ));
		        $this->pdf->row(array('3','PIUTANG DAGANG 61 - 90',
					Yii::app()->numberFormatter->formatCurrency($row9['61sd90']/$per,'Rp. '),
					Yii::app()->numberFormatter->formatCurrency(($row9['61sd90']/($row9['0sd30']+$row9['31sd60']+$row9['61sd90']+$row9['91sd120']+$row9['up120']))*100,''),
		        ));
		        $this->pdf->row(array('4','PIUTANG DAGANG 91 - 120',
					Yii::app()->numberFormatter->formatCurrency($row9['91sd120']/$per,'Rp. '),
					Yii::app()->numberFormatter->formatCurrency(($row9['91sd120']/($row9['0sd30']+$row9['31sd60']+$row9['61sd90']+$row9['91sd120']+$row9['up120']))*100,''),
		        ));
		        $this->pdf->row(array('5','PIUTANG DAGANG > 120',
					Yii::app()->numberFormatter->formatCurrency($row9['up120']/$per,'Rp. '),
					Yii::app()->numberFormatter->formatCurrency(($row9['up120']/($row9['0sd30']+$row9['31sd60']+$row9['61sd90']+$row9['91sd120']+$row9['up120']))*100,''),
		        ));
		        $this->pdf->setFont('Arial','B',10);
