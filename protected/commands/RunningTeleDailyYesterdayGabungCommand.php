<?php
class RunningTeleDailyYesterdayGabungCommand extends CConsoleCommand
{
	protected $pdf;
	public function run($args)
	{
		...
		try
		{
			...
			$connection = Yii::app()->db;
			$this->pdf->title='MONITORING REPORT GABUNGAN';
			$datetime = new DateTime(date($enddate));
			...
			foreach($dataReader as $row)
			{
				...
				$sql1 = "select ifnull(sum(qty),0) as qty
								...
								where b0.recordstatus = 3 and b0.invoiceno is not null and b1.companyid in ({$companyid}) and b0.invoiceno is not null and b4.materialgroupid in (select o.materialgroupid from materialgroup o where o.parentmatgroupid = {$row['materialgroupid']}) and b0.invoicedate = '". date(Yii::app()->params['datetodb'], strtotime($enddate))."') z
								union
								...
								where d5.companyid in ({$companyid}) and d1.recordstatus = 3 and d7.materialgroupid in (select o.materialgroupid from materialgroup o where o.parentmatgroupid = {$row['materialgroupid']}) and d3.gireturdate = '". date(Yii::app()->params['datetodb'], strtotime($enddate))."') z) zz
				";
			...
			foreach($plant as $rows)
			{
				$sqlpenc = "select sum(netto1) as netto, sum(netto2) as netto2, invoicedate from
								...
								c.companyid in ({$companyid}) and h.sloccode like '%%' and d.fullname like '%%' and
								...
								where h.companyid in ({$companyid}) and b.recordstatus = 3 and j.sloccode like '%%' 
			...
			$sql10 = "select sum(a.cashamount + a.bankamount)
							...
							where b.recordstatus=3 and b.companyid in ({$companyid}) and b.docdate = '".date(Yii::app()->params['datetodb'], strtotime($enddate))."'
			";
			...
			$sql11 = "select sum(a.cashamount + a.bankamount)
							from cutarinv a
							join cutar b on b.cutarid=a.cutarid
							where b.recordstatus=3 and b.companyid in ({$companyid}) and b.docdate between '".date(Yii::app()->params['datetodb'], strtotime($startdate))."' and '".date(Yii::app()->params['datetodb'], strtotime($enddate))."'
			";
			...
			$this->pdf->text(14,$this->pdf->gety()+5,'PERSEDIAAN FG   :'.Yii::app()->numberFormatter->formatCurrency($totfg/$per,'Rp. '));
			$this->pdf->text(14,$this->pdf->gety()+10,'PERSEDIAAN WIP :'.Yii::app()->numberFormatter->formatCurrency($totwip/$per,'Rp. '));
			$this->pdf->text(14,$this->pdf->gety()+15,'PERSEDIAAN RW  :'.Yii::app()->numberFormatter->formatCurrency($totrw/$per,'Rp. '));
      ...
      $this->pdf->checkPageBreak(20);
			...
			$this->pdf->Output('/usr/share/nginx/html/agemlive/downloads/DailyMonitoringReportGabungan_AKAGroup_'.$date.'.pdf','F');
			ob_clean();


			$filepath = realpath('/usr/share/nginx/html/agemlive/downloads/DailyMonitoringReportGabungan_AKAGroup_'.$date.'.pdf');
