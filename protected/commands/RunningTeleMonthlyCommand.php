<?php
class RunningTeleMonthlyCommand extends CConsoleCommand
{
	protected $pdf;
	public function run($args)
	{
		...
		try
		{
			...
			$telegroupAccINTIid = "-1001434148313"; //group Top Management
...
$this->pdf->SetProtection(array('print'),$pass,$passmaster);
	...
	*/			
				...
				$sqlcall = "call InsertBSLajur('" . $companyid . "','" . date(Yii::app()->params['datetodb'], strtotime($date)) . "')";
			    Yii::app()->db->createCommand($sqlcall)->execute();
...
$this->pdf->SetProtection(array('print'),$pass,$passmaster);
			    ...
			    $this->pdf->companyid = $companyid;
			    $this->pdf->Cell(0, 0, 'Laporan Neraca', 0, 0, 'C');
			    $this->pdf->Cell(-277, 10, 'Per : ' . date("d F Y", strtotime($date)), 0, 0, 'C');
			    $i = 0;
			          ...
			          array(
						...
						if (($data['accactivaid'] == null) && ($data['accpasivaid'] !== null)) {
							$this->pdf->row(array(
								...
								'',
								'  '.$data['accpasivaname'],
								Yii::app()->format->formatCurrency($data['pasblninitotal']/$per),
								Yii::app()->format->formatCurrency($data['pasblninipersen']),
								Yii::app()->format->formatCurrency($data['pasblnlalutotal']/$per),
							...
							$this->pdf->row(array(
								'  '.$data['accactivaname'],
								Yii::app()->format->formatCurrency($data['accblninitotal']/$per),
								Yii::app()->format->formatCurrency($data['accblninipersen']),
								Yii::app()->format->formatCurrency($data['accblnlalutotal']/$per),
			    ...
			    //$this->pdf->Output();
				$this->pdf->Output('/usr/share/nginx/html/agemlive/downloads/BS_'.$comcode.'_'.$date.'.pdf','F');
				ob_clean();


				$filepath = realpath('/usr/share/nginx/html/agemlive/downloads/BS_'.$comcode.'_'.$date.'.pdf');
