<?php
class RunningwadailyyesterdaywhatsvaV4Command extends CConsoleCommand
{
	public function run($args)
	{
		...
		try
		{
			...
			$groupexcellent = '6285265644828-1461984841';
			$grouptop = '';
			$groupaka = '6285265644828-1483401177';
			$groupks = '';
			$groupamin = '6287875097026-1527279238';
			$groupakm = '6285265644828-1547688422';
			$groupajm = '6281378010952-1533358281';
			$groupami = '6285265644828-1557388538';
			$groupagem = '';
			$groupakp = '6285265644828-1539936063';
			
			$sql1 = "select a.companyid
							from company a
							where a.recordstatus = 1
							and a.nourut > 0
							-- and a.companyid in (7,14)
			...
			foreach($dataReader1 as $row1)
			{
				...
				$title = "DAILY MONITORING REPORT".$display;
...
$msg = "\nPersediaan FG : ".Yii::app()->numberFormatter->formatCurrency($fg['saldoakhir']/$per,"Rp.")
				."\nPersediaan WIP : ".Yii::app()->numberFormatter->formatCurrency($wip['saldoakhir']/$per,"Rp.")
				."\nPersediaan RW: ".Yii::app()->numberFormatter->formatCurrency($rw['saldoakhir']/$per,"Rp.");
$pesan .= $msg;
...
/*				//url dan kirim data untuk wa japri ke Ius Tan
				...
				curl_setopt_array($ch, array(
					CURLOPT_URL => "https://v4.whatsva.com/api/sendText?id_device=22&tujuan=".$whatsvano."@s.whatsapp.net&message=".urlencode($pesan."\n\n_*Dikirim Otomatis oleh SIAGA (System Information AKA Group - Automatic)*_\n".$time)."&id_user=44",
			...
			//url dan kirim data untuk wa group
				...
				$ch = curl_init();
				curl_setopt_array($ch, array(
					CURLOPT_URL => "https://v4.whatsva.com/api/sendText?id_device=22&tujuan=".$developgroup."@g.us&message=".urlencode($pesan."\n\n_*Dikirim Otomatis oleh SIAGA (System Information AKA Group - Automatic)*_\n".$time)."&id_user=44",
					  CURLOPT_RETURNTRANSFER => true,
					  CURLOPT_ENCODING => "",
					  CURLOPT_MAXREDIRS => 10,
					  CURLOPT_TIMEOUT => 0,
...
*/
			//url dan kirim data untuk wa group
				$ch = curl_init();
				curl_setopt_array($ch, array(
					CURLOPT_URL => https://v4.whatsva.com/api/sendMessageGroup?id_device=22&tujuan=".$groupexcellent."@g.us&message=".urlencode($pesan."\n\n_*Dikirim Otomatis oleh SIAGA (System Information AKA Group - Automatic)*_\n".$time)."&id_user=44",
					  CURLOPT_RETURNTRANSFER => true,
					  CURLOPT_ENCODING => "",
					  CURLOPT_MAXREDIRS => 10,
					  CURLOPT_TIMEOUT => 0,
				...
				foreach($dataUser as $wabm)
				{
					...
					$ch = curl_init();
					curl_setopt_array($ch, array(
						CURLOPT_URL => "https://v4.whatsva.com/api/sendText?id_device=22&tujuan=".$wabm['wanumber']."@s.whatsapp.net&message=".urlencode($pesan."\n\n_*Dikirim Otomatis oleh SIAGA (System Information AKA Group - Automatic)*_\n".$time."\n\nUsername:".$wabm['username'])."&id_user=44",
						  CURLOPT_RETURNTRANSFER => true,
						  CURLOPT_ENCODING => "",
						  CURLOPT_MAXREDIRS => 10,
						  CURLOPT_TIMEOUT => 0,
					...
					$res = curl_exec($ch);
					echo $wabm['username']." ".$wabm['wanumber']." ".$res."\n";
				}
				
				
				//japri ke Chief Accounting Whatsva
				$sqluser = "select distinct replace(wanumber,'+','') as wanumber,a.username
							from useraccess a
							join usergroup b on b.useraccessid = a.useraccessid
							join groupaccess c on c.groupaccessid = b.groupaccessid
				...
				foreach($dataUser as $waca)
				{
					
					//url dan kirim data untuk wa japri
					$ch = curl_init();
					curl_setopt_array($ch, array(
						CURLOPT_URL => "https://v4.whatsva.com/api/sendText?id_device=22&tujuan=".$waca['wanumber']."@s.whatsapp.net&message=".urlencode($pesan."\n\n_*Dikirim Otomatis oleh SIAGA (System Information AKA Group - Automatic)*_\n".$time."\n\nUsername:".$waca['username'])."&id_user=44",
						  CURLOPT_RETURNTRANSFER => true,
						  CURLOPT_ENCODING => "",
						  CURLOPT_MAXREDIRS => 10,
						  CURLOPT_TIMEOUT => 0,
