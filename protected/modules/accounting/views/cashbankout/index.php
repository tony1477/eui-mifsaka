...
function downpdfcashbankout() {
	...
	window.open('<?php echo $this->createUrl('cashbankout/downpdf') ?>?id='+ss);
}
				...
				options:{
                        ...
                        onChange:function(newValue,oldValue) {
                            if((newValue !== oldValue) && (newValue !== ''))
                            {
                                var tr = $(this).closest('tr.datagrid-row');
								var index = parseInt(tr.attr('datagrid-row-index'));
								var payamount = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"payamount"});
								var amountold = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"amountold"});
								var chequeid = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"chequeid"});
								var currencyrate = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"currencyrate"});
                                ...
                                jQuery.ajax({
                                    ...
                                    success:function(data)
                                    {
                                        ...
                                        $(payamount.target).numberbox('setValue',data.rows[0].amount);
			...
			options:{
                ...
                onChange:function(newValue,oldValue) {
                    if((newValue !== oldValue) && (newValue !== ''))
                    {
                        var tr = $(this).closest('tr.datagrid-row');
                        var index = parseInt(tr.attr('datagrid-row-index'));
                        var amount = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"amount"});
                        var amountold = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"amountold"});
                        var chequeid = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"chequeid"});
                        var currencyrate = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"currencyrate"});
                        var tglcair = $("#dg-cbapinv").datagrid("getEditor", {index: index, field:"tglcair"});
