...
<div id="tb-balancesheet">
	...
	<select class="easyui-combogrid" id="dlg_search_companyid" name="dlg_search_companyid" style="width:250px" data-options="
        ...
        <a href="javascript:void(0)" title="PDF Neraca-Uji Coba" class="easyui-linkbutton" iconCls="icon-pdf" plain="true" onclick="downpdfneracaujicoba()"></a>
        <a href="javascript:void(0)" title="Excel Neraca-Uji Coba" class="easyui-linkbutton" iconCls="icon-xls" plain="true" onclick="downxlsneracaujicoba()"></a>
...
function downpdfneracaujicoba() {
	...
	var bsdate = $('#dlg_search_bsdate').datebox('getValue');
	var companyid = $('#dlg_search_companyid').combogrid('getValue');
	window.open('<?php echo $this->createUrl('//accounting/balancesheet/downneracaujicobapdf') ?>?company='+companyid+'&date='+bsdate+'&per='+10);
}
function downxlsneracaujicoba() {
	var ss = [];
	var rows = $('#dg-balancesheet').edatagrid('getSelections');
	for(var i=0; i<rows.length; i++){
		var row = rows[i];
	...
	var bsdate = $('#dlg_search_bsdate').datebox('getValue');
	var companyid = $('#dlg_search_companyid').combogrid('getValue');
	window.open('<?php echo $this->createUrl('//accounting/balancesheet/downneracaujicobaxls') ?>?company='+companyid+'&date='+bsdate+'&per='+10);
}
