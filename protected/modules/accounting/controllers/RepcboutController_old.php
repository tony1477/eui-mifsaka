...
class RepcboutController extends Controller
{
  ...
  public function search()
  {
    ...
    $cmd             = Yii::app()->db->createCommand()->select('count(1) as total')->from('cashbankout t')
		...
		->where("(coalesce(b.companyname,'') like :companyid) and
						...
						(coalesce(a.reqpayno,'') like :reqpayid) and t.companyid in (".getUserObjectValues('company').")", array(
      ':companyid' => '%' . $companyid . '%',
      ':docdate' => '%' . $docdate . '%',
  ...
  public function actionSearchinvoice()
  {
    ...
    $result = array_merge($result, array(
      'rows' => $row
		...
		sum(a.payamount+t.payamount) as paymount,
  ...
  public function actionDownxls()
  {
    ...
    $i          = 1;
    $excel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(0, 1, GetCatalog('docdate'))->setCellValueByColumnAndRow(1, 1, GetCatalog('reqpayid'))->setCellValueByColumnAndRow(2, 1, GetCatalog('recordstatus'));
    foreach ($dataReader as $row1) {
      $excel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(0, $i + 1, $row1['docdate'])->setCellValueByColumnAndRow(1, $i + 1, $row1['reqpayid'])->setCellValueByColumnAndRow(2, $i + 1, $row1['recordstatus']);
      $i += 1;
    }
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename="cashbankout.xlsx"');
