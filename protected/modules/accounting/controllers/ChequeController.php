...
class ChequeController extends Controller
{
	...
	public function search()
	{
		...
		$result        = array();
		$row           = array();
		$cmd = Yii::app()->db->createCommand()->select('count(1) as total')->from('cheque t')->join('addressbook a', 'a.addressbookid=t.addressbookid')->join('currency b', 'b.currencyid=t.currencyid')->join('bank c', 'c.bankid=t.bankid')->join('company d', 'd.companyid=t.companyid')->where("(coalesce(t.chequeid,'') like :chequeid) or (coalesce(t.chequeno,'') like :chequeno) or (coalesce(d.companyname,'') like :companyid) or (coalesce(c.bankname,'') like :bankid)
		...
		$result['total'] = $cmd;
		$cmd = Yii::app()->db->createCommand()->select('t.*,a.fullname,b.currencyname,c.bankname,d.companyname, e.plantcode')->from('cheque t')->join('addressbook a', 'a.addressbookid=t.addressbookid')->join('currency b', 'b.currencyid=t.currencyid')->join('bank c', 'c.bankid=t.bankid')->join('company d', 'd.companyid=t.companyid')->leftjoin('plant e', 'e.plantid=t.plantid')->where("(coalesce(t.chequeid,'') like :chequeid) or (coalesce(t.chequeno,'') like :chequeno) or (coalesce(d.companyname,'') like :companyid) or (coalesce(c.bankname,'') like :bankid)
			and t.recordstatus <> 0 and
			t.companyids in (" . getUserObjectValues('company') . ")", array(
			':chequeid' => '%' . $chequeid . '%',
			':chequeno' => '%' . $chequeno . '%',
			':companyid' => '%' . $companyid . '%',
			':bankid' => '%' . $bankid . '%',
