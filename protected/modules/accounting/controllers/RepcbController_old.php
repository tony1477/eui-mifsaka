...
class RepcbController extends Controller
{
  ...
  public function actionSearchacc()
  {
    ...
    $result          = array();
    $row             = array();
    $cmd             = Yii::app()->db->createCommand()->select('count(1) as total')->from('cbacc t')->join('account a', 'a.accountid=t.debitaccid')->join('account b', 'b.accountid=t.creditaccid')->join('currency c', 'c.currencyid=t.currencyid')->leftjoin('cheque d', 'd.chequeid=t.chequeid')->leftjoin('employee e', 'e.employeeid=t.employeeid')->leftjoin('addressbook f', 'f.addressbookid=t.customerid')->leftjoin('addressbook g', 'g.addressbookid=t.supplierid')->where('cbid = :cbid', array(
      ':cbid' => $id
    ))->queryScalar();
    $result['total'] = $cmd;
    $cmd             = Yii::app()->db->createCommand()->select('t.*,d.chequeid,d.chequeno,a.accountid as debitaccid,b.accountid as creditaccid,a.accountname as accdebitname,b.accountname as acccreditname,c.currencyname,e.fullname,f.fullname as customername,g.fullname as suppliername')->from('cbacc t')->join('account a', 'a.accountid=t.debitaccid')->join('account b', 'b.accountid=t.creditaccid')->join('currency c', 'c.currencyid=t.currencyid')->leftjoin('cheque d', 'd.chequeid=t.chequeid')->leftjoin('employee e', 'e.employeeid=t.employeeid')->leftjoin('addressbook f', 'f.addressbookid=t.customerid')->leftjoin('addressbook g', 'g.addressbookid=t.supplierid')->where('cbid = :cbid', array(
      ':cbid' => $id
    ))->queryAll();
    foreach ($cmd as $data) {
      $row[] = array(
