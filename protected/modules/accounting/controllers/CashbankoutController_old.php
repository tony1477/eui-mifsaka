  ...
  public function search() {
    ...
    $cmd             = Yii::app()->db->createCommand()->select('count(1) as total')->from('cashbankout t')
		...
		->where("(coalesce(b.companyname,'') like :companyid) and
						...
						(coalesce(a.reqpayno,'') like :reqpayid) and t.recordstatus in (".getUserRecordStatus('listcbout').") and t.companyid 
  ...
  public function actionSaveinvoice() {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        ...
        $sql     = 'call Updatecbapinv(:vid,:vcashbankoutid,:vplantid,:vinvoiceapid,:vekspedisiid,:vaccountid,:vcashbankno,:vtglcair,:vpayamount,:vcurrencyid,:vcurrencyrate,:vbankaccountno,:vbankname,:vbankowner,:vitemnote,:vcreatedby)';
        $command = $connection->createCommand($sql);
