...
class GenjournalController extends Controller {
		...
		if (move_uploaded_file($_FILES["file-genjournal"]["tmp_name"], $target_file)) {
			...
			try {
				for ($row = 2; $row <= $highestRow; ++$row) {
					...
					if ($abid == '') {					
						$this->ModifyData($connection,array('',$companyid,$journaldate,$noref,$journalnote));
  ...
  public function actionSave() {
	...
	private function ModifyDataDetail($connection,$arraydata) {
		$id = (isset($arraydata[0])?$arraydata[0]:'');
		if ($id == '') {
			$sql     = 'call Insertjournaldetail(:vgenjournalid,:vaccountid,:vdebit,:vcredit,:vcurrencyid,:vratevalue,:vdetailnote,:vcreatedby)';
			$command = $connection->createCommand($sql);
		} else {
			$sql     = 'call Updatejournaldetail(:vid,:vgenjournalid,:vaccountid,:vdebit,:vcredit,:vcurrencyid,:vratevalue,:vdetailnote,:vcreatedby)';
			$command = $connection->createCommand($sql);
			$command->bindvalue(':vid', $arraydata[0], PDO::PARAM_STR);
			$this->DeleteLock($this->menuname, $arraydata[0]);
		}
  ...
  public function actionSavedetail() {
    ...
    $transaction = $connection->beginTransaction();
    try {
			$this->ModifyDataDetail($connection,array((isset($_POST['journaldetailid'])?$_POST['journaldetailid']:''),$_POST['genjournalid'],$_POST['accountid'],
