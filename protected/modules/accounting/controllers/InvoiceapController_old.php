...
class InvoiceapController extends Controller {
  ...
  public function search() {
    ...
    if (isset($_GET['list'])) {
      ...
      ((t.invoicedate like :invoicedate) or 
                            ...
                            (b.fullname like :addressbookid)) and t.recordstatus < 3 and f.companyid in (".getUserObjectValues('company').")", array(
      ...
      $cmd = Yii::app()->db->createCommand()->select('count(1) as total')->from('invoiceap t')->leftjoin('poheader a', 'a.poheaderid=t.poheaderid')->leftjoin('addressbook b', 'b.addressbookid=t.addressbookid')->leftjoin('currency c', 'c.currencyid=t.currencyid')->leftjoin('paymentmethod d', 'd.paymentmethodid=t.paymentmethodid')->leftjoin('tax e', 'e.taxid=t.taxid')->leftjoin('company f', 'f.companyid = t.companyid')->leftjoin('grheader g', 'g.grheaderid = t.grheaderid')->where("((t.invoicedate like :invoicedate) or 
                            (t.invoiceno like :invoiceno) or 
                            (a.pono like :poheaderid) or
                            (b.fullname like :addressbookid)) and t.recordstatus = 3
						...
						where k.recordstatus=6 and k.companyid=t.companyid)", array(
        ':invoicedate' => '%' . $invoicedate . '%',
        ':invoiceno' => '%' . $invoiceno . '%',
        ':poheaderid' => '%' . $poheaderid . '%',
        ':addressbookid' => '%' . $addressbookid . '%'
      ...
      $cmd = Yii::app()->db->createCommand()->select('count(1) as total')->from('invoiceap t')
		...
		->where("(coalesce(t.invoicedate,'') like :invoicedate) and 
			...
			(coalesce(b.fullname,'') like :addressbookid)
					and t.recordstatus in (".getUserRecordStatus('listinvap').") and 
                    t.companyid in (".getUserObjectWfValues('company','listinvap').")", array(
        ':invoicedate' => '%' . $invoicedate . '%',
  ...
  public function actionSavematerial() {
    ...
    //try {
      $id = (isset($arraydata[0])?$arraydata[0]:'');
			if ($id == '') {
        $sql     = 'call Insertinvoiceapjurnal(:vinvoiceapid,:vaccountid,:vdebet,:vcredit,:vcurrencyid,:vcurrencyrate,:vdescription,:vcreatedby)';
        $command = $connection->createCommand($sql);
      } else {
        $sql     = 'call Updateinvoiceapjurnal(:vid,:vinvoiceapid,:vaccountid,:vdebet,:vcredit,:vcurrencyid,:vcurrencyrate,:vdescription,:vcreatedby)';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $arraydata[0], PDO::PARAM_STR);
      }
      $command->bindvalue(':vinvoiceapid', $arraydata[1], PDO::PARAM_STR);
      $command->bindvalue(':vaccountid', $arraydata[2], PDO::PARAM_STR);
      $command->bindvalue(':vdebet', $arraydata[3], PDO::PARAM_STR);
      $command->bindvalue(':vcredit', $arraydata[4], PDO::PARAM_STR);
      $command->bindvalue(':vcurrencyid', $arraydata[5], PDO::PARAM_STR);
      $command->bindvalue(':vcurrencyrate', $arraydata[6], PDO::PARAM_STR);
