<table id="dg-reportout" style="width:auto;height:400px">
</table>
<div id="tb-reportout">
	<?php
 if (checkaccess($this->menuname, $this->iswrite) == 1) {  ?>
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="javascript:$('#dg-reportout').edatagrid('addRow')"></a>
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-save',plain:true" onclick="javascript:$('#dg-reportout').edatagrid('saveRow')"></a>
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-undo" plain="true" onclick="javascript:$('#dg-reportout').edatagrid('cancelRow')"></a>
	<?php }?>
	<?php if (checkaccess($this->menuname, $this->ispurge) == 1) {  ?>
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-purge" plain="true" onclick="javascript:$('#dg-reportout').edatagrid('destroyRow')"></a>
<?php }?>
	<?php if (checkaccess($this->menuname, $this->isdownload) == 1) {  ?>
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-pdf" plain="true" onclick="downpdfreportout()"></a>
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-xls" plain="true" onclick="downxlsreportout()"></a>
<?php }?>

	<input class="easyui-searchbox" data-options="prompt:'Please Input Value',searcher:searchreportout" style="width:150px">
</div>

<script type="text/javascript">
$('#dg-reportout').edatagrid({
		iconCls: 'icon-edit',	
		singleSelect: false,
		toolbar:'#tb-reportout',
		pagination: true,
		fitColumns:true,
		ctrlSelect:true,
		autoSave:true,
    url: '<?php echo $this->createUrl('reportout/index',array('grid'=>true)) ?>',
    saveUrl: '<?php echo $this->createUrl('reportout/save',array('grid'=>true)) ?>',
    updateUrl: '<?php echo $this->createUrl('reportout/save',array('grid'=>true)) ?>',
    destroyUrl: '<?php echo $this->createUrl('reportout/purge',array('grid'=>true)) ?>',
		onSuccess: function(index,row){
			show('Message',row.msg);
			$('#dg-reportout').edatagrid('reload');
		},
...
,
		...
		{
field:'reportoutid',
title:'<?php echo getCatalog('reportoutid') ?>',
sortable: true,
...
function searchreportout(value){
	$('#dg-reportout').edatagrid('load',{
	reportoutid:value,
employeeid:value,
fullname:value,
fulldivision:value,
...
function downpdfreportout() {
	var ss = [];
	var rows = $('#dg-reportout').edatagrid('getSelections');
	for(var i=0; i<rows.length; i++){
			var row = rows[i];
			ss.push(row.reportoutid);
	}
	window.open('<?php echo $this->createUrl('reportout/downpdf') ?>?id='+ss);
}
function downxlsreportout() {
	var ss = [];
	var rows = $('#dg-reportout').edatagrid('getSelections');
	for(var i=0; i<rows.length; i++){
			var row = rows[i];
	...
	window.open('<?php echo $this->createUrl('reportout/downxls') 
