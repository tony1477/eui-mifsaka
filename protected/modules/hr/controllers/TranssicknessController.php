<?php
class TranssicknessController extends Controller
{
	public $menuname = 'transsickness';
	...
	public function search()
	{
		...
		// search 
		$transsicknessid = isset ($_POST['transsicknessid']) ? $_POST['transsicknessid'] : '';
		...
		$transsicknessid = isset ($_GET['q']) ? $_GET['q'] : $transsicknessid;
		...
		$sort = isset($_POST['sort']) ? strval($_POST['sort']) : 'transsicknessid';
		...
		{	
			$row[] = array(
                        'transsicknessid'=>$data['transsicknessid'],
                        ...
                        'recordstatus'=>findstatusname("apptranssickness",$data['recordstatus']),
			);
		}
	...
	public function actionSave()
	{
		...
		try
		{
			...
			{
				$sql = 'call Inserttranssickness(:vdocdate,:vemployeeid,:vstartdate,:venddate,:vdescription,:vrecordstatus,:vcreatedby)';
				$command=$connection->createCommand($sql);
			}
			else
			{
				$sql = 'call Updatetranssickness(:vid,:vdocdate,:vemployeeid,:vstartdate,:venddate,:vdescription,:vrecordstatus,:vcreatedby)';
				$command=$connection->createCommand($sql);
				$command->bindvalue(':vid',$_POST['transsicknessid'],PDO::PARAM_STR);
				$this->DeleteLock($this->menuname, $_POST['transsicknessid']);
			}
	...
	public function actionPurge()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				$sql = 'call Purgetranssickness(:vid,:vcreatedby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
	...
	public function actionApprove()
	{
		...
		if (isset($_POST['id']))
		{
			...
			try
			{
				$sql = 'call ApproveTranssickness(:vtranssicknessid,:vlastupdateby)';
				$command=$connection->createCommand($sql);
				foreach($id as $ids)
				{
					$command->bindvalue(':vtranssicknessid',$ids,PDO::PARAM_STR);
	...
	public function actionDownPDF()
	{
	  ...
	  $sql = "select docdate,employeeid,startdate,enddate,description,recordstatus
				from transsickness a ";
		if ($_GET['id'] !== '') {
				$sql = $sql . "where a.transsicknessid in (".$_GET['id'].")";
		}
		$command=$this->connection->createCommand($sql);
		$dataReader=$command->queryAll();

		//masukkan judul
		$this->pdf->title=getCatalog('transsickness');
		$this->pdf->AddPage('P');
