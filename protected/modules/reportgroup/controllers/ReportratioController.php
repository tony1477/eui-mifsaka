<?php
class ReportRatioController extends Controller
{
  public $menuname = 'groupratio';
  ...
  public function actionSave()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        ...
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $_POST['repneracaid'], PDO::PARAM_STR);
        $this->DeleteLock($this->menuname, $_POST['repneracaid']);
      }
      $command->bindvalue(':vcompanyid', $_POST['companyid'], PDO::PARAM_STR);
      $command->bindvalue(':vaccountid', $_POST['accountid'], PDO::PARAM_STR);
      $command->bindvalue(':visdebet', $_POST['isdebet'], PDO::PARAM_STR);
      $command->bindvalue(':vaccformula', $_POST['accformula'], PDO::PARAM_STR);
      $command->bindvalue(':vaftacc', $_POST['aftacc'], PDO::PARAM_STR);
  ...
  public function actionDownPDF()
  {
    ...
    $connection = Yii::app()->db;
    //$sql        = "call InsertBSLajur('" . $_GET['company'] . "','" . date(Yii::app()->params['datetodb'], strtotime($_GET['date'])) . "')";
    //$this->connection->createCommand($sql)->execute();
    $sql        = "select a.*
			from repneracalajur a 
			where a.companyid = " . $_GET['company'] . " 
  ...
  public function actionDownRatioPDF()
  {
    ...
    $akt  = $akt_company ."
            from repneracalajur a 
			where a.tahun = year('" . date(Yii::app()->params['datetodb'], strtotime($_GET['date'])) . "')
			and a.bulan = month('" . date(Yii::app()->params['datetodb'], strtotime($_GET['date'])) . "')
			".$where;
    $pas  = $pas_company ."
			from repneracalajur a 
    ...
    $sql18 = "select sum(debit)
              ...
              where year(b.journaldate) = year('".date(Yii::app()->params['datetodb'], strtotime($_GET['date']))."')
