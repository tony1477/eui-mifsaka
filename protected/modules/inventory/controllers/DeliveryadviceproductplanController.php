<?php
class DeliveryadviceproductplanController extends Controller
{
  public $menuname = 'deliveryadviceproductplan';
  ...
  public function search()
  {
    ...
    $productplanid    = isset($_GET['q']) ? $_GET['q'] : $productplanid;
  ...
  public function actionsearchdetail()
  {
    ...
    $cmd             = Yii::app()->db->createCommand()->select('t.*,a.productname,b.uomcode,c.requestedbycode,d.sloccode,d.description,getstock(t.productid,t.unitofmeasureid,t.slocid) as stock')
    	->from('deliveryadvicedetail t')->leftjoin('product a', 'a.productid = t.productid')
    	->leftjoin('unitofmeasure b', 'b.unitofmeasureid = t.unitofmeasureid')
    	->leftjoin('requestedby c', 'c.requestedbyid = t.requestedbyid')
    	->leftjoin('sloc d', 'd.slocid = t.slocid')->where('deliveryadviceid = :deliveryadviceid', array(
      ':deliveryadviceid' => $id
    ))->offset($offset)->limit($rows)->order($sort . ' ' . $order)->queryAll();
    foreach ($cmd as $data) {
      $row[] = array(
  ...
  public function actionSave()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        ...
        $sql     = 'call UpdateDAPP(:vid,:vdadate,:vheadernote,:vslocid,:vproductplanid,:vcreatedby)';
        $command = $connection->createCommand($sql);
  ...
  public function actionDownPDF()
  {
    ...
    foreach ($dataReader as $row) {
      ...
      $this->pdf->text(90, $this->pdf->gety(), 'SPP ');
      $this->pdf->text(100, $this->pdf->gety(), ': ' . $row['productplanno']);
  ...
  public function actionDownxls()
  {
    ...
    $line       = 3;
    foreach ($dataReader as $row) {
      $this->phpExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(0, $line, 'No')->setCellValueByColumnAndRow(1, $line, ': ' . $row['dano'])->setCellValueByColumnAndRow(4, $line, 'SPP')->setCellValueByColumnAndRow(5, $line, ': ' . $row['productplanno']);
      $line++;
