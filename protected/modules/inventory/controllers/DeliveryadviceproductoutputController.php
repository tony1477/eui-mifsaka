<?php
class DeliveryadviceproductoutputController extends Controller
{
  public $menuname = 'deliveryadviceproductoutput';
  ...
  public function actionGenerateop()
  {
    if (isset($_POST['id'])) {
      $connection  = Yii::app()->db;
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call GenerateDAOP(:vid, :vslocid,:vhid)';
  ...
  public function search()
  {
    ...
    $productoutputid  = isset($_GET['q']) ? $_GET['q'] : $productoutputid;
    ...
    if (isset($_GET['list'])) {
      $cmd = Yii::app()->db->createCommand()->select('count(1) as total')->from('deliveryadvice t')->where('
				...
				(t.dano like :dano) or
				(t.productplanno like :productplanid) or
				(t.productoutputno like :productoutputid) or				
    ...
    if (isset($_GET['list'])) {
      ...
      $cmd = Yii::app()->db->createCommand()->select('t.*')->where("((t.dano like :dano) or (b.sloccode like :slocid) or (t.slocdesc like :slocid)) 
				and t.recordstatus = {$maxstat}
				and t.recordstatus in (".getUserRecordStatus('listda').")
        and t.useraccessid in (".getUserObjectValues('useraccess').")
        and t.deliveryadviceid in (select dad.deliveryadviceid
      ...
      $cmd = Yii::app()->db->createCommand()->select('t.*')->from('deliveryadvice t')->where("((t.dano like :dano) or (b.sloccode like :slocid) or (t.slocdesc like :slocid)) 
				and t.recordstatus = {$maxstat}
				and t.recordstatus in (".getUserRecordStatus('listda').")
        and t.useraccessid in (".getUserObjectValues('useraccess').")
        and t.deliveryadviceid in (select dad.deliveryadviceid
      ...
      $cmd = Yii::app()->db->createCommand()->select('t.*,
		...
		) as warna')->from('deliveryadvice t')->where("(
				(coalesce(t.deliveryadviceid,'') like :deliveryadviceid) and
				(coalesce(t.dadate,'') like :dadate) and
				(coalesce(t.dano,'') like :dano) and
				(coalesce(t.productoutputno,'') like :productoutputid) and				
  ...
  public function actionSave()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        ...
        $sql     = 'call UpdateDAOP(:vid,:vdadate,:vheadernote,:vslocid,:vproductoutputid,:vcreatedby)';
        $command = $connection->createCommand($sql);
  ...
  public function actionDownPDF()
  {
    ...
    $this->pdf->title = getCatalog('deliveryadviceproductoutput');
    ...
    foreach ($dataReader as $row) {
      ...
      $this->pdf->text(90, $this->pdf->gety(), 'OP ');
      $this->pdf->text(100, $this->pdf->gety(), ': ' . $row['productoutputno']);
      ...
      $this->pdf->text(50, $this->pdf->gety() + 10, '........................');
      $this->pdf->text(120, $this->pdf->gety() + 10, '........................');
      $this->pdf->text(170, $this->pdf->gety() + 10, '........................');
    }
    $this->pdf->Output();
  }
  public function actionDownxls()
  {
    $this->menuname = 'deliveryadviceproductoutput';
    ...
    $line       = 3;
    foreach ($dataReader as $row) {
      $this->phpExcel->setActiveSheetIndex(0)->setCellValueByColumnAndRow(0, $line, 'No')->setCellValueByColumnAndRow(1, $line, ': ' . $row['dano'])->setCellValueByColumnAndRow(4, $line, 'OP')->setCellValueByColumnAndRow(5, $line, ': ' . $row['productoutputno']);
      $line++;
