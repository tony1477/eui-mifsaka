...
class TransstockController extends Controller
{
  ...
  public function search()
  {
    ...
    $row              = array();
        ...
        and t.slocfromid in (".getUserObjectValues('sloc').")
				and t.slocfromid in (".getUserObjectWfValues('sloc','appts').")
        ";
  ...
  public function actionGeneratedetail()
  {
    if (isset($_POST['id'])) {
      $cmd         = Yii::app()->db->createCommand()->select('t.slocid as slocfromid,t.headernote,
					a.slocid as sloctoid')->from('deliveryadvice t')->leftjoin('deliveryadvicedetail a', 'a.deliveryadviceid = t.deliveryadviceid')->where('t.deliveryadviceid = ' . $_POST['id'])->queryRow();
      /*$cmd         = Yii::app()->db->createCommand()->select('t.slocid as slocfromid,t.headernote,
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call GenerateTSDA(:vid,:vslocfrom,:vslocto,:vhid,:vcreatedby)';
        $command = $connection->createCommand($sql);
  ...
  public function actionDownPDF()
  {
    ...
    foreach ($dataReader as $row) {
      ...
      $this->pdf->text(90, $this->pdf->gety(), ': ' . $row['dano']);
