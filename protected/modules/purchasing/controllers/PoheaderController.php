...
class PoheaderController extends Controller {
  ...
  public function actionGetpriceplant() {
    // check detail first
    $detail = Yii::app()->db->createCommand("select ifnull(count(1),0) from podetail where poheaderid = ".$_POST['poheaderid'])->queryScalar();
    if($detail==0) return 0;
    $connection = Yii::app()->db;
    $transaction = $connection->beginTransaction();
    try {
      $q = 'call getpriceplant(:vid,:vplantid)';
      $p = $connection->createCommand($q);
      $p->bindvalue(':vid', $_POST['poheaderid'], PDO::PARAM_STR);
      $p->bindvalue(':vplantid', $_POST['plantid'], PDO::PARAM_STR);
      $p->execute();
      $transaction->commit();
...
-- and z.slocid in (".getUserObjectValues('sloc').")
    ...
    $sqlcount="select count(1) as total
                ...
                join addressbook b on b.fullname = c.companyname
    ...
    $sqldata = " select a.poheaderid,a.pono,c.companyname as customer
                from poheader a
                join company c on c.companyid=a.companyid
                join addressbook b on b.fullname = c.companyname
                where a.recordstatus=5 
  ...
  public function actionSave() {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        $sql     = 'call Insertpoheader(:vpurchasinggroupid,:vdocdate,:vaddressbookid,:vplantid,:vheadernote,:vpono,:vpaymentmethodid,:vprintke,:vshipto,:vbillto,:vcompanyid,:vtaxid,:vrecordstatus,:vcreatedby)';
      ...
      $command->bindvalue(':vplantid', $_POST['plantid']=='' ? 0 : $_POST['plantid'], PDO::PARAM_STR);
