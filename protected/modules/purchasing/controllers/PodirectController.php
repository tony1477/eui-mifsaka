...
class PodirectController extends Controller {
		...
		if (move_uploaded_file($_FILES["FilePodirect"]["tmp_name"], $target_file)) {
			...
			try {
				for ($row = 2; $row <= $highestRow; ++$row) {
					...
					$purchasinggroupid = Yii::app()->db->createCommand("select purchasinggroupid from purchasinggroup where description = '".$purchasinggroup."'")->queryScalar();
					...
					if ($abid == '') {					
						...
						$this->ModifyData($connection,array('',$companyid,$purchasinggroupid,$docdate,$pono,$supplierid,$paymentmethodid,$taxid,$shipto,$billto,$headernote,$recordstatus));
					...
					if ($abid != '') {
						if ($objWorksheet->getCellByColumnAndRow(12, $row)->getValue() != '') {
							...
							$productid = Yii::app()->db->createCommand("select productid from product where recordstatus = 1 and productname = '".$productname."'")->queryScalar();
  ...
  public function actionUploadPO() {
		...
		$target_file = dirname('__FILES__').'/uploads/' . basename($_FILES["FilePodirectPO"]["name"]);
		if (move_uploaded_file($_FILES["FilePodirectPO"]["tmp_name"], $target_file)) {
			$objReader = PHPExcel_IOFactory::createReader('Excel2007');
			$objPHPExcel = $objReader->load($target_file);
			$objWorksheet = $objPHPExcel->getActiveSheet();
			$highestRow = $objWorksheet->getHighestRow(); 
			...
			try {
				for ($row = 1; $row <= $highestRow; ++$row) {
					$companycode = $objWorksheet->getCellByColumnAndRow(1, $row)->getValue();
					$companyid = Yii::app()->db->createCommand("select companyid from company where companycode = '{$companycode}'")->queryScalar();
                    $supplier = $objWorksheet->getCellByColumnAndRow(3, $row)->getValue();
					$addressbookid = Yii::app()->db->createCommand("select addressbookid from addressbook where fullname = '{$supplier}'")->queryScalar();
                    ...
                    $row+=1;
                    $podate = date(Yii::app()->params['datetodb'],strtotime($objWorksheet->getCellByColumnAndRow(1, $row)->getValue()));
                    ...
                    $stmt = Yii::app()->db->createCommand("select getrunno({$companyid},22,'{$podate}')");
                    $pono = $stmt->queryScalar();
                    
                    $this->ModifyData($connection,array('',$companyid,'',
								$podate, $pono, $addressbookid, $paymentmethodid, $taxid, $shipto, $billto,$headernote,1));
                    ...
                    if ($abid != '')
                    {
                        ...
                        for ($i = $row; $i <= $highestRow; ++$i)
                        {
                            if ($objWorksheet->getCellByColumnAndRow(1, $i)->getValue() != '')
                            {
                                ...
                                $underdelv = $objWorksheet->getCellByColumnAndRow(9, $i)->getValue();
                                $overdelv = $objWorksheet->getCellByColumnAndRow(10, $i)->getValue();
                                $description = $objWorksheet->getCellByColumnAndRow(11, $i)->getValue();
                                $this->ModifyDataDetail($connection,array('',$abid,'',$productid,$qty,$uomid,$delvdate,$price,$currencyid,$currencyrate,$slocid,
                                $underdelv,$overdelv,$description));
                                $row++;
                            }
                        }
                    }
  ...
  public function actionSave() {
    ...
    $transaction = $connection->beginTransaction();
    try {
        $this->ModifyData($connection,array((isset($_POST['poheaderid'])?$_POST['poheaderid']:''),
			$_POST['companyid'],
  ...
  public function actionSavedetail() {
    ...
    try {
        ...
        $this->ModifyDataDetail($connection,array((isset($_POST['podetailid'])?$_POST['podetailid']:''),$_POST['poheaderid'],
                '',
                $_POST['productid'],
                $_POST['poqty'],
