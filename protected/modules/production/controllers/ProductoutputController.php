...
class ProductoutputController extends Controller
{
  ...
  public function actionGenerateplan()
  {
    if (Yii::app()->request->isAjaxRequest) {
      ...
      try {
        ...
        $id = Yii::app()->db->createCommand("select ifnull(b.employeeid,0) as employeeid, ifnull(b.fullname,'-') as fullname from productplan a left join employee b on b.employeeid = a.employeeid 
        where a.productplanid = ".$_POST['id'])->queryRow();
        if (Yii::app()->request->isAjaxRequest) {
			echo CJSON::encode(array(
				'status'=>'success',
				'employeeid'=> $id['employeeid'],
				'foremanname'=> $id['fullname'],
				));
			Yii::app()->end();
		}
      }
  ...
  public function search()
  {
    ...
    $description				= isset($_POST['description']) ? $_POST['description'] : '';
		...
		$sql = ' 
			...
			t.statusname,t.recordstatus,t.companyid,b.sono,b.soheaderid,c.fullname,c.addressbookid,t.employeeid, ifnull(d.fullname,"-") as foremanname '.$from.' '.$where;
  ...
  public function actionSave()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        $sql     = 'call Insertproductoutput(:vproductoutputdate,:vproductplanid,:vemployeeid,:vdescription,:vcreatedby)';
        $command = $connection->createCommand($sql);
      } else {
        $sql     = 'call Updateproductoutput(:vid,:vproductoutputdate,:vproductplanid,:vemployeeid,:vdescription,:vcreatedby)';
        $command = $connection->createCommand($sql);
  ...
  public function actionSavedetail()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        ...
        $sql     = 'call Updateproductoutputdetail(:vid,:vstoragebintoid,:vdescription,:vcreatedby)';
        $command = $connection->createCommand($sql);
  ...
  public function actionDownPDF()
  {
    ...
    $sql = "select a.*,b.productplanno,b.productplandate, ifnull(c.fullname,'') as fullname
      from productoutput a 
    ...
    foreach ($dataReader as $row) {
      ...
      if($row['fullname']!='')
      {
          $this->pdf->text(15, $this->pdf->gety() + 15, 'Foreman  ');
          $this->pdf->text(50, $this->pdf->gety() + 15, ' : '.$row['fullname']);
      }
...
LEFT JOIN storagebin d ON d.storagebinid = t.storagebinid 
 ...
 where t.productoutputid = " . $row['productoutputid'] ;
	...
	public function actionDownXLSminus()
  {
      $this->menuname = 'productionminus';
      parent::actionDownxls();
      $sql = "select b.productoutputno, b.productoutputdate, b.productplanno
      from productoutput b ";
      if ($_GET['id'] !== '') {
      ...
      $line=0;
      $this->phpExcel->setActiveSheetIndex(0)
          ->setCellValueByColumnAndRow(0, 0, 'Stock Tidak Mencukupi');
      ...
      foreach ($dataReader as $row) {
        $this->phpExcel->setActiveSheetIndex(0)
            ...
            ->setCellValueByColumnAndRow(4, $line, 'NO SPP :')
            ->setCellValueByColumnAndRow(5, $line, $row['productplanno']);
        $line++;
        $this->phpExcel->setActiveSheetIndex(0)
            ->setCellValueByColumnAndRow(0, $line, 'Tanggal :')
            ->setCellValueByColumnAndRow(1, $line, date(Yii::app()->params['dateviewfromdb'], strtotime($row['productoutputdate'])));
        ...
        $this->phpExcel->setActiveSheetIndex(0)
            ...
            ->setCellValueByColumnAndRow(2, $line, 'Qty Pemakaian')
            ->setCellValueByColumnAndRow(3, $line, 'Qty Stock')
            ->setCellValueByColumnAndRow(4, $line, 'Qty Detail')
        ...
        (
          ...
          from productoutputdetail a
          join product j on j.productid=a.productid
          where a.productoutputid = '".$_GET['id']."' group by toslocid,a.productid
        ) zz
        where zz.qtystock < zz.qty or zz.qtydetail < zz.qty";
        ...
        foreach ($dataReader1 as $row1) {
            ...
            $this->phpExcel->setActiveSheetIndex(0)
                ...
                ->setCellValueByColumnAndRow(3, $line, $row1['qtystock'])
                ->setCellValueByColumnAndRow(4, $line, $row1['qtydetail'])
