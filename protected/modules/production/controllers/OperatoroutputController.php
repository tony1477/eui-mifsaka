<?php
class OperatoroutputController extends Controller
{
  public $menuname = 'operatoroutput';
  ...
  public function actionGetData()
  {
    if (isset($_GET['id'])) {
			...
			$sql = "insert into operatoroutput (opoutputdate,recordstatus,ctover) values ('".$dadate->format('Y-m-d')."',".findstatusbyuser('insopoutput').",0)";
  ...
  public function search()
  {
    ...
    $result           = array();
    $row              = array();
		$maxstat = Yii::app()->db->createCommand("select getwfmaxstatbywfname('appopoutput')")->queryScalar();
    /*
    ...
    */
    if(isset($_GET['combo']))
    {
        $cmd = Yii::app()->db->createCommand()->select('count(1) as total')
                ...
                ->where("((t.opoutputdate like :opoutputdate) or
                        ...
                        and t.recordstatus in (".getUserRecordStatus('listopoutput').") 
    ...
    else
    {
        ...
        ->where("(
				 ...
				 (coalesce(d.fullname,'') like '%".$fullname."%')) and t.recordstatus < {$maxstat} and 
                 t.recordstatus in (".getUserRecordStatus('listopoutput').") and
    ...
    else
    {
        $cmd = Yii::app()->db->createCommand()->selectdistinct('t.*,a.companyname, b.sloccode, concat(b.sloccode,"-",b.description) as slocdesc, getwfstatusbywfname("appopoutput",t.recordstatus) as statusname, e.shiftname, e.shiftcode')
            ...
            ->where("(      (coalesce(b.sloccode,'') like :sloccode) and
                            ...
                            (coalesce(t.operatoroutputid,'') like :operatoroutputid) and
                            (coalesce(t.headernote,'') like :headernote) and
                            (coalesce(d.fullname,'') like '%".$fullname."%')) and t.recordstatus < {$maxstat} and t.recordstatus in (".getUserRecordStatus('listopoutput').") and
                            t.companyid in (".getUserObjectValues('company').")", array(
          ':sloccode' => '%' . $slocid . '%',
          ':operatoroutputid' => '%' . $operatoroutputid . '%',
          ':companyid' => '%' . $companyid . '%',
  ...
  public function actionSave()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        $sql     = 'call Insertoperatsor(:vopoutputdate,:vcompanyid,:vslocid,:vshiftschedid,:vheadernote,:visover,:vctover,:vcreatedby)';
        $command = $connection->createCommand($sql);
      } else {
        $sql     = 'call Updateoperator(:vid,:vopoutputdate,:vcompanyid,:vslocid,:vshiftschedid,:vheadernote,:visover,:vctover,:vcreatedby)';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $_POST['operatoroutputid'], PDO::PARAM_STR);
        $this->DeleteLock($this->menuname, $_POST['operatoroutputid']);
      }
      $command->bindvalue(':vopoutputdate', date(Yii::app()->params['datetodb'], strtotime($_POST['opoutputdate'])), PDO::PARAM_STR);
      $command->bindvalue(':vcompanyid', $_POST['companyid'], PDO::PARAM_INT);
      $command->bindvalue(':vslocid', $_POST['slocid'], PDO::PARAM_INT);
      $command->bindvalue(':vshiftschedid', $_POST['shiftschedid'], PDO::PARAM_INT);
      $command->bindvalue(':vheadernote', $_POST['headernote'], PDO::PARAM_STR);
      $status = isset($_POST['isover']) ? ($_POST['isover'] == "on") ? 1 : 0 : 0;
      $command->bindvalue(':visover', $status, PDO::PARAM_STR);
      $command->bindvalue(':vctover', $_POST['ctover'], PDO::PARAM_STR);
  ...
  public function actionSavedetail()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        $sql     = 'call Insertoperatordet(:voperatoroutputid,:vemployeeid,:vstandardopoutputid,:vqty,:vdescription,:vcreatedby)';
        $command = $connection->createCommand($sql);
      } else {
        $sql     = 'call Updateoperatordet(:vid,:voperatoroutputid,:vemployeeid,:vstandardopoutputid,:vqty,:vdescription,:vcreatedby)';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $_POST['operatoroutputdetid'], PDO::PARAM_STR);
        $this->DeleteLock($this->menuname, $_POST['operatoroutputdetid']);
      }
      $command->bindvalue(':voperatoroutputid', $_POST['operatoroutputid'], PDO::PARAM_STR);
      $command->bindvalue(':vemployeeid', $employeeid, PDO::PARAM_STR);
      $command->bindvalue(':vstandardopoutputid', $_POST['standardopoutputid'], PDO::PARAM_STR);
  ...
  public function actionSaveissue()
  {
    ...
    try {
      if (isset($_POST['isNewRecord'])) {
        $sql     = 'call Insertoperatorissue(:voperatoroutputid,:voperatoroutputdetid,:vstandardissueid,:vdescription,:vcycletime,:vcreatedby)';
        $command = $connection->createCommand($sql);
      } else {
        $sql     = 'call Updateoperatorissue(:vid,:voperatoroutputid,:voperatoroutputdetid,:vstandardissueid,:vdescription,:vcycletime,:vcreatedby)';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $_POST['operatoroutputissueid'], PDO::PARAM_STR);
        $this->DeleteLock($this->menuname, $_POST['operatoroutputissueid']);
      }
      $command->bindvalue(':voperatoroutputid', $_POST['operatoroutputid'], PDO::PARAM_STR);
      $command->bindvalue(':voperatoroutputdetid', $_POST['operatoroutputdetid'], PDO::PARAM_STR);
      $command->bindvalue(':vstandardissueid', $_POST['standardissueid'], PDO::PARAM_STR);
      $command->bindvalue(':vdescription', $_POST['description'], PDO::PARAM_STR);
  ...
  public function actionPurge()
  {
    ...
    if (isset($_POST['id'])) {
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call Purgecbs(:vid,:vcreatedby)';
        $command = $connection->createCommand($sql);
        foreach ($id as $ids) {
  ...
  public function actionPurgedetail()
  {
    ...
    if (isset($_POST['id'])) {
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call PurgeOpoutput(:vid,:vcreatedby)';
        $command = $connection->createCommand($sql);
  ...
  public function actionPurgeissue()
  {
    header("Content-Type: application/json");
    if (isset($_POST['id'])) {
      $id          = $_POST['id'];
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call PurgeOpoutputissue(:vid,:vcreatedby)';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $id, PDO::PARAM_STR);
        $command->bindvalue(':vcreatedby', Yii::app()->user->name, PDO::PARAM_STR);
        $command->execute();
  ...
  public function actionDelete()
  {
    ...
    if (isset($_POST['id'])) {
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call DeleteOpOutput(:vid,:vcreatedby)';
        $command = $connection->createCommand($sql);
        foreach ($id as $ids) {
          $command->bindvalue(':vid', $ids, PDO::PARAM_STR);
          $command->bindvalue(':vcreatedby', Yii::app()->user->name, PDO::PARAM_STR);
