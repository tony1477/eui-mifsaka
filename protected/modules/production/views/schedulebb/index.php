<table id="dg-schedulebb" style="width:100%;height:97%"></table>
<div id="tb-schedulebb">
	<?php
 if (CheckAccess($this->menuname, $this->iswrite) == 1) {  ?>
		<a href="javascript:void(0)" title="Tambah"class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="javascript:$('#dg-schedulebb').edatagrid('addRow')"></a>
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-save',plain:true" onclick="javascript:$('#dg-schedulebb').edatagrid('saveRow')"></a>
		<a href="javascript:void(0)" title="Kembali"class="easyui-linkbutton" iconCls="icon-undo" plain="true" onclick="javascript:$('#dg-schedulebb').edatagrid('cancelRow')"></a>
	<?php }?>
	<?php if (CheckAccess($this->menuname, $this->ispurge) == 1) {  ?>
		<a href="javascript:void(0)" title="Hapus"class="easyui-linkbutton" iconCls="icon-purge" plain="true" onclick="javascript:$('#dg-schedulebb').edatagrid('destroyRow')"></a>
<?php }?>
	<?php if (CheckAccess($this->menuname, $this->isdownload) == 1) {  ?>
		<a href="javascript:void(0)" title="Export Ke PDF"class="easyui-linkbutton" iconCls="icon-pdf" plain="true" onclick="downpdfschedulebb()"></a>
		<a href="javascript:void(0)" title="Export Ke EXCEL"class="easyui-linkbutton" iconCls="icon-xls" plain="true" onclick="downxlsschedulebb()"></a>
<?php }?>
   <form enctype="multipart/form-data" method="post" id="ff">
<table>
<tr>
<td><?php echo getCatalog('product')?></td>
<td><input class="easyui-textbox" id="schedulebb_search_product" style="width:150px"></td>
<td><?php echo getCatalog('company')?></td>
<td><input class="easyui-textbox" id="schedulebb_search_company" style="width:150px"></td>
<td><?php echo getCatalog('sloc')?></td>
<td><input class="easyui-textbox" id="schedulebb_search_sloc" style="width:150px"><a href="javascript:void(0)" title="Cari"class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="searchschedulebb()"></a></td>
</tr>
    <tr>
<td>Upload File</td>
<td>
...
$(document).ready(function(){
    $('#ff').form({
        url:'production/schedulebb/uploaddata',
        ajax:'true',
        dataType: 'json',
        data : $("#ff").serialize(),
        ...
        success: function(data){
            ...
            if (data.status == "success"){
                jQuery.ajax({'url':'production/schedulebb/running',
...
$('#dg-schedulebb').edatagrid({
		iconCls: 'icon-edit',	
		singleSelect: false,
		toolbar:'#tb-schedulebb',
		pagination: true,
		fitColumns:true,
		ctrlSelect:true,
    autoSave:false,
    url: '<?php echo $this->createUrl('schedulebb/index',array('grid'=>true)) ?>',
    saveUrl: '<?php echo $this->createUrl('schedulebb/save',array('grid'=>true)) ?>',
    updateUrl: '<?php echo $this->createUrl('schedulebb/save',array('grid'=>true)) ?>',
    destroyUrl: '<?php echo $this->createUrl('schedulebb/purge',array('grid'=>true)) ?>',
		onSuccess: function(index,row){
			show('Message',row.msg);
			$('#dg-schedulebb').edatagrid('reload');
		},
		onError: function(index,row){
			show('Message',row.msg);
		},
		idField:'schedulebbid',
		editing: '<?php echo ($this->CheckAccess($this->menuname, $this->iswrite) == 1 ? 'true' : 'false') ?>',
		columns:[[
		{
field:'schedulebbid',
...
function searchschedulebb(value){
	$('#dg-schedulebb').edatagrid('load',{
			schedulebbid:value,
			product:value,
			sloc:value,
			company:value,
...
function downpdfschedulebb() {
	var ss = [];
	var rows = $('#dg-schedulebb').edatagrid('getSelections');
	for(var i=0; i<rows.length; i++){
			var row = rows[i];
			ss.push(row.schedulebbid);
	}
	window.open('<?php echo $this->createUrl('schedulebb/downpdf') ?>?id='+ss);
}
function downxlsschedulebb() {
	var ss = [];
	var rows = $('#dg-schedulebb').edatagrid('getSelections');
	for(var i=0; i<rows.length; i++){
			var row = rows[i];
	...
	window.open('<?php echo $this->createUrl('schedulebb/downxls') 
