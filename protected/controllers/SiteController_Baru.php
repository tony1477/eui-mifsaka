...
class SiteController extends Controller
{
  ...
  public function actionSaveProfile()
  {
    ...
    if ($model['password'] !== $_POST['password']) {
      Yii::app()->db->createCommand("update useraccess set password = md5('" . $_POST['password'] . "') where username = '" . $_POST['username'] . "'")->execute();
			/*$sql = "CALL inserttranslogex('".$_POST['username']."','update','".md5($_POST['password'])."','".$model['password']."','useraccess',".$model['useraccessid'].",'".$this->getip()."','test')";*/
    ...
    $this->GetMessage(true, 'insertsuccess');
  }
  ...
  public function actionusertodo()
  {
    ...
    // result	
    $cmd = Yii::app()->db->createCommand()->select('count(1) as total')->from('usertodo t')->where("username = '" . Yii::app()->user->name . "'")->queryRow();
