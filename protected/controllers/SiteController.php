...
class SiteController extends Controller {
  ...
  {
    $items = array();
    $cmd   = Yii::app()->db->createCommand()->selectdistinct("a.fullname, round(datediff(last_day('".$_GET['start']."'),a.birthdate)/365,0) as umur, d.companycode, a.birthdate, replace(a.birthdate,year(a.birthdate),year('".$_GET['start']."')) as startdate")->from('employee a')->leftjoin('employeeorgstruc b', 'b.employeeid = a.employeeid')->leftjoin('orgstructure c', 'c.orgstructureid = b.orgstructureid')->leftjoin('company d', 'd.companyid = c.companyid')->where("month(a.birthdate) between month('".$_GET['start']."') and month(last_day('".$_GET['end']."')) and b.recordstatus = 1 ")->queryAll();
    
    foreach ($cmd as $data) {
      $items[] = array(
        'title' => $data['fullname'] . " \n Umur : " . $data['umur']. " \n Perusahaan : " . $data['companycode'],
  ...
  /*
  public function actiongetcompareppop()
  {
      header("content-type: application/json"); 
     
      $sql = "select a.delvdate,d.fullname,group_concat(c.productname) as productname,a.qty,b.pocustno,e.uomcode
            from sodetail a
            ...
            where a.delvdate between '2018-11-01' and '2018-12-01' and b.recordstatus in (".getUserRecordStatus('listso').") 
            and b.companyid in (".getUserObjectValues('company').")
            group by b.soheaderid,b.addressbookid limit 5";
...
*/
  public function actiondeliveryschedule()
  {
    ...
    $sql = "select a.delvdate,d.fullname,group_concat(c.productname) as productname,a.qty,b.pocustno,e.uomcode
            ...
            join addressbook d on d.addressbookid= b.addressbookid
            join unitofmeasure e on e.unitofmeasureid = a.unitofmeasureid	
            where a.delvdate between '".$_GET['start']."' and '".$_GET['end']."' and b.recordstatus in (".getUserRecordStatus('listso').") 
            and b.companyid in (".getUserObjectValues('company').")
            group by b.soheaderid,b.addressbookid";
    $cmd = Yii::app()->db->createCommand($sql)->queryAll();
