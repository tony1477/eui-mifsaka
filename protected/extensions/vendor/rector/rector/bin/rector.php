...
use RectorPrefix20211123\Symfony\Component\Console\Input\ArgvInput;
...
$autoloadIncluder = new \RectorPrefix20211123\AutoloadIncluder();
...
try {
    ...
    // for json output
    $argvInput = new \RectorPrefix20211123\Symfony\Component\Console\Input\ArgvInput();
    ...
    if ($outputFormat === \Rector\ChangesReporting\Output\JsonOutputFormatter::NAME) {
        echo \RectorPrefix20211123\Nette\Utils\Json::encode(['fatal_errors' => [$throwable->getMessage()]]);
    } else {
        // report fatal errors in console format
        $symfonyStyleFactory = new \Rector\Core\Console\Style\SymfonyStyleFactory(new \RectorPrefix20211123\Symplify\PackageBuilder\Reflection\PrivatesCaller());
        $symfonyStyle = $symfonyStyleFactory->create();
        $symfonyStyle->error($throwable->getMessage());
    }
    exit(\RectorPrefix20211123\Symfony\Component\Console\Command\Command::FAILURE);
}
...
\class_alias('RectorPrefix20211123\\AutoloadIncluder', 'AutoloadIncluder', \false);
